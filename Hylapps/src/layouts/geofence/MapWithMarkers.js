...
const createCustomIcon = (heading, width, height, iconType) => {
  ...
  switch (iconType) {
    ...
    case 'large':
      ...
      break;
    case 'extra-large':
      iconUrl = '/ship-popup.png'; // Extra-large icon URL
      break;
    default:
...
const getIconForZoom = (zoom) => {
  ...
  if (zoom > 13.75) return { width: 10, height: 10, type: 'point-icon'   };
  if (zoom > 12.75) return { width: 10, height: 10, type: 'point-icon'   };
  if (zoom > 11.5) return { width: 9, height: 9, type: 'point-icon'  };
  if (zoom > 10.75) return { width: 8, height: 8, type: 'point-icon' };
  if (zoom > 9.75) return { width:8, height: 8, type: 'point-icon' };
  if (zoom > 8.75) return { width: 8, height: 8, type: 'point-icon'  };
  if (zoom > 7) return { width: 8, height: 8, type: 'point-icon' };
  if (zoom > 6) return { width: 8, height: 8, type: 'point-icon'  };
  if (zoom > 4) return { width: 8, height: 8, type: 'point-icon'  };
  if (zoom > 2) return { width: 7, height: 7, type: 'point-icon'};
  return { width: 5, height: 5, type: 'point-icon'};
};
